...
geordi$meta[geordi$new.category %in% c("like-post", "unfavorite", "report-post","subscribe")] <- "impressions"
...
# activity summary 
geordi_vis <- ddply(subset(geordi,userID =="(anonymous)"), c("meta","new.category"),summarize, 
                           users = length(unique(userID_a)),
                           activities = length(userID_a))

# plot 
ggbarplot(subset(geordi_vis, !meta %in% c("classification","impressions","learn")), x = "new.category", y = "users",
...
p <- ggdotchart(subset(geordi_vis, !meta %in% c("classification","impressions","learn")), x = "new.category", y = "activities",
