...
geordi <- geordi[,-c(3,8,10:17,19)] # remove unnecessary columns 
# max time for record "2017-04-28 15:00:16 UTC"

# First logged in as indicated by first classification with a username
joindate <- ddply(geordi,"userID",summarize,
                  firstactivity =min(time[userID != "(anonymous)"]))
...
geordi <- geordi[,-c(1,3,6:9,11,12)] # remove unnecessary columns 


# import comments from system logs
comments <- read_csv("~/Dropbox/INSPIRE/Data/System dumps/GS Comments/gravity-spy-comments_2018-05-23.csv") 
comments <- comments[which(comments$comment_created_at <= "2017-04-28"),]
comments <- comments[,-c(1:4,6:11,13)] # remove unnecessary columns 
names(comments)[2] <- "userID_a"
comments$clientIP <- ""
comments$userID <- comments$userID_a
comments$new.category <- paste0("post-",comments$board_title)
comments$board_title <- NULL
names(comments)[2] <- "time"

comments <- comments[,c(3,2,4,1,5)]
# remove post comment event  
geordi <- geordi[which(!geordi$new.category %in% c("post-comment")),]
geordi <- rbind(geordi,comments)
remove(comments)
# remove comment category and replace with actual comment posts from system logs

# reproduce the original dataset
geordi_org <- geordi
...
geordi$meta[geordi$new.category %in% c("post-Bug reports","edit-post","delete-post","private-message",
                                       "update-comment","create-discussion"," post-Chat","post-Collections",
                                       "post-Help","post-Notes","post-Science")] <- "socialize"
...
# remove events that weren't important. This is that dataset to be used in the analysis
...
geordi_anonymous_nolink <- geordi[which(is.na(geordi$userID_a)),] 
...
# simple bar plot 
m <- ggbarplot(subset(geordi_visall, !meta %in% c("classification")), x = "new.category", y = "activities",
...
p <- ggbarplot(subset(geordi_vislinked, !meta %in% c("classification","impressions","learn")), x = "new.category", y = "activities",
           fill = "meta",               # change fill color by cyl
          color = "white",            # Set bar border colors to white
          palette = "jco",            # jco journal color palett. see ?ggpar
          sort.val = "asc",                              # Add segments from y = 0 to dots
           rotate = TRUE,                                # Rotate vertically
           group = "meta",                                # Order by groups
           #dot.size = 6,                                 # Large dot size
           label = "users",         # Add mpg values as dot labels
           font.label = list(color = "black", size = 2),               # Adjust label parameters
          ggtheme = theme_minimal(),
           ylab = "Event Count",xlab = "Event"
           )
p <- p + font("xy.text", size = 9) 

## Indication events before a user's first registered activity 
geordi <- merge(geordi,joindate, by.y=("userID"), by.x=("userID_a"), all.x = TRUE)
geordi$preevent <- ifelse(geordi$time < geordi$firstactivity,1,0) #1 if the record is before registering
...
geordi_summary <- ddply(geordi, c("userID_a"), summarize, 
                        ...
                        first_anonactivity = min(time[userID=="(anonymous)"]),
                        first_regactivitiy = min(time[userID!="(anonymous)"]),
                        ...
                        classifications = length(userID_a[new.category=="classify"])
                        )
geordi_summary$timepre <- geordi_summary$first_regactivitiy - geordi_summary$first_anonactivity
...
# export and generate sessions
setwd("~/Dropbox/INSPIRE/Papers & Presentations/Anonymous Work (Journal)/Data")
#write.csv(geordi,"geordi2.csv")
# generate new sessions and re-import data
geordi2 <- read_csv("~/Dropbox/INSPIRE/Papers & Presentations/Anonymous Work (Journal)/Data/geordiIP_linksession.csv")

colnames(geordi2)[15] <- "Sequence"
geordi2[c(1,2,11,12,13)] <- NULL
```
...
geordi_summary_nolink$hms <- strftime(geordi_summary_nolink$first_activity, format = "%H")

timecount <- ddply(geordi_summary_nolink, c("hms"), summarize,
                   count = length(clientIP))

# plot dates
ses_start <- ggbarplot(timecount, x = "hms", y = "count", 
          fill = "#00AFBB", 
          palette = "jco",
          font.label = list(color = "white", size = 7, 
                             vjust = 0.5),               # Adjust label parameters
           ylab = "No. Sessions",xlab = "Session Start Time",
          ggtheme = theme_pubr()) 
...
geordi2_summary <- ddply(geordi2, c("userID_a","Session"), summarize,
                         ...
                         anonviewing = length(which(new.category %in% c(
                           ...
                           "talk-view","talk-newglitches-view","talk-collections-view")  
                            & userID == "(anonymous)")),
                         anonsocialize = length(which(new.category %in% c("post-Bug reports","edit-post","delete-post","private-message",
                                       "update-comment","create-discussion"," post-Chat","post-Collections",
                                       "post-Help","post-Notes","post-Science")
                           & userID == "(anonymous)")),
...
z <- ggviolin(geordi2_user_summary_future, x = "Anon.Type", y = "classifications",
...
nextactivity$meta[nextactivity$new.category %in% c("talk-bugs-view","talk-chat-view", "talk-help-view",
                                "talk-science-view","talk-notes-view","talk-view","talk-newglitches-view",
                                "talk-collections-view")] <- "viewing"
nextactivity$meta[nextactivity$new.category %in% c("post-Bug reports","edit-post","delete-post","private-message",
                                       "update-comment","create-discussion"," post-Chat","post-Collections",
                                       "post-Help","post-Notes","post-Science")] <- "socialize"
