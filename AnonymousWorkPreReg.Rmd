...
# import datasets 
geordi <- read_csv("~/Documents/RA/Code/geordi-1-5-17.csv") # The raw geordi dataet
...
events <- read_csv("~/Documents/RA/Code/events.csv")
...
anonymous_activities_nolink <- count(geordi_anonymous_nolink$new.category[which(geordi_anonymous_nolink$userID=="(anonymous)")])

colnames(anonymous_activities_nolink)[1] <- "event"

anonymous_activities_nolink$event <- factor(anonymous_activities_nolink$event, levels=unique(as.character(anonymous_activities_nolink$event)) )
anonymous_activities_nolink <- transform(anonymous_activities_nolink, event=reorder(event, -freq) )

nolinkanon <- ggplot(subset(anonymous_activities_nolink, event != "classify"), aes(event, freq)) + 
  geom_bar(stat = "identity", fill="dodgerblue4") + 
  coord_flip() + 
  labs(x="Event Category",y="Frequency", title="Anonymous Events (Not Linked)") + 
  theme(
    axis.text.x=element_text(size=9),
    axis.text.y=element_text(size=9)
  )
...
#summarizing 
geordi_summary1 <- ddply(geordi_anonymous_nolink, c("clientIP"), summarize, 
                        activities = length(clientIP),
                        classifications = length(clientIP[new.category == "classify"]),
                        non_classifications = length(clientIP[new.category != "classify"]),
                        percentage_classify = classifications/activities*100
                        )
```
```{r}
