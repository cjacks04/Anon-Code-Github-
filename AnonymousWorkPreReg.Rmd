...
geordi_anonymous_nolink <- geordi[which(is.na(geordi$userID_a)),] #117862 no link

### USE THIS DATASET FOR REPORTING RESULTS
geordi <- geordi[which(!is.na(geordi$userID_a)),] #1840853
...
All anonymous events for which we were able to link to a user account. There is another dataset that has 117862 events that we weren't able to link to users.
...
```{r}
allanon # All anonymous events...not classifications
```

### Analysis: Anonymous events that we were not able to link to a user account

There are 117,862 events that we weren't able to link to users.
```{r}
#geordi_anonymous_nolink 
